...
\begin{itemize}
    ...
    \item \textbf{Stage 3}: Terjadi pertumbuhan pembuluh darah baru ke arah vitreous di
...
tortuositas pembuluh darah di kutub posterior retina, yang dapat menyertai \textit{stage}
...
\begin{figure}[H]
  ...
  \caption{Empat kategori kelas retina pada klasifikasi ROP: Normal, Stage 1 dengan \textit{demarcation
  line}, Stage 2 dengan \textit{ridge}, dan Stage 3 dengan neovaskularisasi ekstraretina.
...
Dataset sekunder yang digunakan dalam penelitian ini adalah HVDROPDB dari \citet{Agrawal2021},
khususnya bagian HVDROPDB-BV yang menyediakan 100 pasang citra fundus bayi beserta masker
pembuluh darah yang dianotasi secara manual. Dataset ini mencakup 50 citra dari kamera RetCam
dan 50 citra dari kamera Neo, sehingga mencerminkan variasi perangkat pengambilan gambar yang
umum digunakan dalam skrining ROP.

%--------------------------------------------------------------
\section{Praproses dan \textit{Enhancement} Citra Fundus}
%--------------------------------------------------------------
...
%--------------------------------------------------------------
\section{Segmentasi Pembuluh Darah Retina}
%--------------------------------------------------------------

Segmentasi pembuluh darah retina secara otomatis adalah langkah praproses yang penting dalam
analisis citra fundus untuk berbagai penyakit, termasuk ROP. Pembuluh darah retina memiliki
karakteristik geometris yang khas, yaitu struktur \textit{tubular} dengan intensitas yang lebih
rendah dibanding jaringan sekitarnya pada kanal hijau citra RGB.
...
nilai eigen dari matriks Hessian orde dua untuk mengukur derajat \textit{vesselness} pada
...
\subsection{Pendekatan Fusi Multi-Detektor}

Penelitian terbaru menunjukkan bahwa menggabungkan respons dari beberapa jenis detektor dapat
meningkatkan kualitas segmentasi secara lebih efektif daripada mengoptimalkan satu detektor
saja. Filter Gabor lebih baik dalam mendeteksi pembuluh darah besar yang lebih tebal, sementara
filter berbasis Hessian seperti Meijering lebih sensitif terhadap pembuluh darah kecil yang
tipis. Fusi keduanya dengan pembobotan yang tepat dapat menghasilkan peta segmentasi yang
mencakup rentang ukuran pembuluh darah yang lebih luas \citep{Almeida2024}.
...
\citet{He2016} merupakan fondasi dari sebagian besar model \textit{deep learning} modern.
...
\textit{focal loss} sebagai alternatif \textit{cross-entropy} standar yang secara otomatis
mengurangi bobot pada sampel yang mudah diklasifikasikan sehingga pelatihan lebih berfokus pada
sampel yang sulit, yang biasanya merupakan sampel dari kelas minoritas. Pendekatan berbasis
ensemble seperti \textit{Random Forest} juga mendukung mekanisme pembobotan kelas secara
native \citep{Breiman2001}.
