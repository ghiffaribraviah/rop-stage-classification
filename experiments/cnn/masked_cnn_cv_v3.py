...
def train_one_fold(df, tr_idx, te_idx, cache, device, epochs, smoothing, ema_decay,
                   ...
                   stage1_oversample=1.0, seed=42):
    random.seed(seed); np.random.seed(seed); torch.manual_seed(seed)
    tr_df, te_df = df.iloc[tr_idx], df.iloc[te_idx]
    tr_ds = CacheDataset(cache, tr_df, augment=True)
    if stage1_oversample > 1.0:
        sw = np.where(tr_df['label_id'].values == STAGE1_ID, stage1_oversample, 1.0).astype(np.float64)
        sampler = WeightedRandomSampler(torch.from_numpy(sw), num_samples=len(sw), replacement=True)
        tl = DataLoader(tr_ds, 32, sampler=sampler, num_workers=4, drop_last=True)
    else:
        tl = DataLoader(tr_ds, 32, shuffle=True, num_workers=4, drop_last=True)
    ...
    with torch.no_grad():
        for imgs, _ in el:
            ...
            if tta:
                views += [torch.flip(imgs, dims=[2]),
                          torch.flip(imgs, dims=[2, 3]),
                          torch.rot90(imgs, 1, dims=[2, 3]),
                          torch.rot90(imgs, 3, dims=[2, 3])]
            probs = sum(torch.softmax(net(v), dim=1) for v in views) / len(views)
            logits_all.append(torch.log(probs.clamp_min(1e-8)).cpu().numpy())
...
def run_full(protocol="group", epochs=160, smoothing=0.1, ema_decay=0.999, mix_p=0.5,
    ...
    for fold, (tr, te) in enumerate(splits):
        lg, _ = train_one_fold(df, tr, te, cache, device, epochs, smoothing, ema_decay,
                               ...
                               stage1_oversample=stage1_oversample)
        oof_logits[te] = lg
    ...
    tag = (f"5-fold OOF [{protocol}] ep={epochs} ls={smoothing} eps={eps_ord} "
           f"cap={weight_cap} rank={use_rank_aux} mixp={mix_p} tta={tta} s1os={stage1_oversample}")
    ...
    # Detached .spawn() runs have no local client to capture the return value; persist to Volume.
    try:
        fname = f"v3_full_result_s1os{stage1_oversample}.json" if stage1_oversample > 1.0 else "v3_full_result.json"
        (cache / fname).write_text(json.dumps(out, indent=2))
        cache_vol.commit()
        print(f"Saved result to /cache/{fname}", flush=True)
    except Exception as e:
        print(f"WARN: failed to persist result to volume: {e}", flush=True)
    return out

@app.function(image=image, volumes={"/cache": cache_vol}, gpu="A100-80GB", timeout=18000)
def run_ensemble(protocol="group", epochs=160, smoothing=0.1, ema_decay=0.999, mix_p=0.5,
                 tta=True, eps_ord=0.15, weight_cap=4.0, use_rank_aux=False, sweep_stage1=True,
                 n_seeds=3, max_folds=5, seed_base=42):
    """Honest seed-ensemble over the OOF protocol: for each fold, train n_seeds models
    on that fold's TRAIN data only and average their probability-space logits on the
    fold's OWN held-out slice. No held-out image is ever seen in training, so the OOF
    contract holds and the number generalizes."""
    root = Path("/root/data/Zhao2024"); cache = Path("/cache/masked3ch_v2")
    df = load_manifest(root); print(f"Loaded {len(df)}: {df['label'].value_counts().to_dict()}", flush=True)
    ensure_cache(df, cache)
    device = torch.device('cuda'); X = np.arange(len(df)); y = df['label_id'].values
    ...
    if protocol == "group":
        skf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=42)
        splits = list(skf.split(X, y, groups=_build_groups(df)))
    else:
        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42); splits = list(skf.split(X, y))
    for fold, (tr, te) in enumerate(splits):
        if fold >= max_folds: break
        ens = np.zeros((len(te), len(CLASSES)), dtype=np.float64)
        for s in range(n_seeds):
            lg, _ = train_one_fold(df, tr, te, cache, device, epochs, smoothing, ema_decay,
                                   mix_p, tta, eps_ord, weight_cap, use_rank_aux,
                                   seed=seed_base + s)
            ens += np.exp(lg.astype(np.float64))
            sf = precision_recall_fscore_support(y[te], lg.argmax(1), average='macro', zero_division=0)[2]
            print(f"  Fold {fold} seed {seed_base + s}: macro-F1={sf:.4f}", flush=True)
        ens_logits = np.log(np.clip(ens / n_seeds, 1e-8, None)).astype(np.float32)
        oof_logits[te] = ens_logits; covered[te] = True
        ff = precision_recall_fscore_support(y[te], ens_logits.argmax(1), average='macro', zero_division=0)[2]
        print(f"Fold {fold} ENSEMBLE({n_seeds}): macro-F1={ff:.4f} (n={len(te)})", flush=True)

    m = covered
    tag = (f"{int(covered.sum())}/{len(df)} OOF [{protocol}] ENS={n_seeds} ep={epochs} "
           f"eps={eps_ord} cap={weight_cap} tta={tta}")
    res = _report(df.iloc[m].reset_index(drop=True), oof_logits[m].argmax(1), tag + " | raw-argmax")
    out = {'raw': res, 'tag': tag, 'n_seeds': n_seeds, 'max_folds': max_folds}
    if sweep_stage1:
        bias, base_f1, best_f1 = sweep_stage1_bias(oof_logits[m], y[m])
        adj = oof_logits[m].copy(); adj[:, STAGE1_ID] += bias
        res2 = _report(df.iloc[m].reset_index(drop=True), adj.argmax(1), tag + f" | stage1_bias={bias:+.3f}")
        print(f"Stage1 sweep: bias={bias:+.3f}  base_f1={base_f1:.4f} -> swept_f1={best_f1:.4f}", flush=True)
        out['swept'] = res2; out['stage1_bias'] = bias
    print("RESULT_JSON=" + json.dumps(out), flush=True)
    try:
        fname = f"v3_ensemble_ns{n_seeds}_mf{max_folds}.json"
        (cache / fname).write_text(json.dumps(out, indent=2)); cache_vol.commit()
        print(f"Saved result to /cache/{fname}", flush=True)
    except Exception as e:
        print(f"WARN: failed to persist result to volume: {e}", flush=True)
    return out
...
def main(mode: str = "quick", protocol: str = "group", epochs: int = 160,
         ...
         stage1_oversample: float = 1.0, n_seeds: int = 3, max_folds: int = 5):
    if mode == "quick":
        quick_test.remote(epochs=min(epochs, 40), eps_ord=eps_ord,
                          weight_cap=weight_cap, use_rank_aux=rank_aux)
    elif mode == "ensemble":
        run_ensemble.remote(protocol=protocol, epochs=epochs, eps_ord=eps_ord,
                            weight_cap=weight_cap, use_rank_aux=rank_aux,
                            n_seeds=n_seeds, max_folds=max_folds)
    else:
