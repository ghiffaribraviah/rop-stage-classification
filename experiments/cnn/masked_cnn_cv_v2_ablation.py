...
app = modal.App("rop-masked-cnn-cv-v2-ablation")
...
def build_ordinal_soft_targets(labels, n_classes, eps_ord, smoothing):
    ...
    for i in range(B):
        ...
        if c == LASER_ID:
            t[i, c] = 1.0 - smoothing
            continue
        neigh = [m for m in (c - 1, c + 1) if m in ord_set]
...
def sweep_stage1_bias(oof_logits, y, lo=-3.0, hi=1.0, steps=81):
    ...
    for bdelta in np.linspace(lo, hi, steps):
        ...
        f1 = precision_recall_fscore_support(y, adj.argmax(1), average='macro', zero_division=0)[2]
        if f1 > best_f1:
...
def train_one_fold(df, tr_idx, te_idx, cache, device, epochs, smoothing, ema_decay, mix_p, tta, eval_model="ema",
                   use_ordinal=False, eps_ord=0.15):
    ...
    for ep in range(1, epochs + 1):
        ...
        for imgs, lbls in tl:
            ...
            opt.zero_grad()
            if use_ordinal:
                onsce_mixed(model(imgs), a, b, lam, w, eps_ord, smoothing).backward()
            else:
...
def run_full(protocol="group", epochs=160, smoothing=0.1, ema_decay=0.999, mix_p=0.5, tta=True,
             use_ordinal=False, eps_ord=0.15, max_folds=5):
    ...
    for fold, (tr, te) in enumerate(splits):
        ...
        lg, _ = train_one_fold(df, tr, te, cache, device, epochs, smoothing, ema_decay, mix_p, tta,
                               use_ordinal=use_ordinal, eps_ord=eps_ord)
        oof_logits[te] = lg; covered[te] = True
    ...
    m = covered
    cfg = f"ordinal={use_ordinal}" + (f"(eps={eps_ord})" if use_ordinal else "")
    tag = f"{int(covered.sum())}/{len(df)} OOF [{protocol}] ep={epochs} ls={smoothing} {cfg}"
    res = _report(df.iloc[m].reset_index(drop=True), oof_logits[m].argmax(1), tag + " | raw-argmax")
    out = {'raw': res, 'tag': tag, 'use_ordinal': use_ordinal, 'eps_ord': eps_ord}
...
def main(mode: str = "", protocol: str = "group", epochs: int = 160,
         eps_ord: float = 0.15, max_folds: int = 5):
    if mode == "ablation":
        print("=== CONFIG A: v2 base (no ordinal) -> raw + swept ===", flush=True)
        run_full.remote(protocol=protocol, epochs=epochs, use_ordinal=False,
                        eps_ord=eps_ord, max_folds=max_folds)
        print("=== CONFIG B: v2 + ordinal (ONSCE) -> raw + swept ===", flush=True)
        run_full.remote(protocol=protocol, epochs=epochs, use_ordinal=True,
                        eps_ord=eps_ord, max_folds=max_folds)
    elif mode == "base":
        run_full.remote(protocol=protocol, epochs=epochs, use_ordinal=False,
                        eps_ord=eps_ord, max_folds=max_folds)
        ...
        print("No mode. Use --mode ablation | base | ordinal [--epochs N --max-folds K]", flush=True)
